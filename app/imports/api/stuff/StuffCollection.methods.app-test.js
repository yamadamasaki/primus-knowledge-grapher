...
import { stuffDefineMethod, stuffRemoveItMethod, stuffUpdateMethod } from './StuffCollection.methods'
...
if (Meteor.isClient) {
  describe('StuffCollection Meteor Methods', function testSuite() {
    it('Can define, update, and removeIt', async function test1() {
      ...
      expect(doc.name)
          .to
          .equal(definitionData.name)
      expect(doc.quantity)
          .to
          .equal(definitionData.quantity)
      expect(doc.condition)
          .to
          .equal(definitionData.condition)
      const updateData = {}
      updateData.id = docID
      ...
      expect(doc.name)
          .to
          .equal(updateData.name)
      expect(doc.quantity)
          .to
          .equal(updateData.quantity)
      expect(doc.condition)
          .to
          .equal(updateData.condition)
