  ...
  describe('StuffItemAdmin', function testSuite() {
    it('should render', function test1() {
      ...
      const wrapper = mount(<table>
        <tbody><StuffItemAdmin stuff={item}/></tbody>
      </table>)
      ...
      expect(wrapper.find('td')
          ...
          .text())
          .to
          .equal(name) // name Table.Cell
      expect(wrapper.find('td')
          .at(1)
          .text())
          .to
          .equal(`${quantity}`) // quantity Table.Cell
      expect(wrapper.find('td')
          .at(2)
          .text())
          .to
          .equal(condition) // condition Table.Cell
      expect(wrapper.find('td')
          .at(3)
          .text())
          .to
          .equal(owner) // edit Table.Cell
    })
  })
}
